...
def get_data(
    ...
    """
    Get data for a list of stocks using rd.get_data()
    """
    # Validate inputs
    if not stocks:
    ...
    request_params = parameters.copy() if parameters else {}
    request_params["SDate"] = start_date
    request_params["EDate"] = end_date
    if frequency:
        request_params["Frq"] = frequency
    ...
    print("=" * 60)
    print(f"Starting data loading (rd.get_data)")
    ...
    for i, stock in enumerate(remaining_stocks.copy()):
        ...
        stock_data = get_data_with_retry(
            stock=stock,
            fields=fields,
...
def get_data_with_retry(stock, fields, parameters, max_retries=3, retry_delay=5):
    """
    ...
    for attempt in range(1, attempts + 1):
        try:
            data = rd.get_data(stock, fields, parameters=parameters)
            ...
            if attempt < attempts:
                print(f"  Getting data in {retry_delay} seconds...")
                time.sleep(retry_delay)
    
    print(f"  Failed to get data for {stock} after {attempts} attempts")
    return None
